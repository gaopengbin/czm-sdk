...
function setString(str: string, len: number) {
  var strlen = 0;
  var s = "";
...
export default class MarkerManager {
  [x: string]: any;
  constructor(viewer: Cesium.Viewer, markerOptions = CesiumBillboard.defaultStyle,
  ...
  pick(type = "marker", mode = "single") {
    ...
    const updateTip = function (e: any) {
  ...
  createMarker(cartesian: Cesium.Cartesian3) {
  ...
  createLabel(cartesian: Cesium.Cartesian3) {
  ...
  select(type: string, id: string, status: boolean) {
  ...
  import(feat: any) {
    ...
    if (feat.properties.mtype === GraphicType.LABEL) {
      const lopts: any = CesiumLabel.defaultStyle;
