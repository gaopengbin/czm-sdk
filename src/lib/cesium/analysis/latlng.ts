 ...
 */
latlng.getCurrentMousePosition = function (scene: Cesium.Scene, position: Cesium.Cartesian2, noPickEntity: Cesium.Entity) {
    var cartesian;
 ...
 * */
latlng.getCenter = function (viewer: Cesium.Viewer) {
...
function pickCenterPoint(scene: Cesium.Scene) {
 ...
 * @param {Cesium.Viewer} viewer 
 * */
latlng.getExtent = function (viewer: Cesium.Viewer) {
    // 范围对象
    var extent = {
        xmin: 70,
        xmax: 140,
 ...
 * 
 * */
latlng.getViewBounds = function (viewer: Cesium.Viewer) {
 ...
 * 
 * */
latlng.getCameraView = function (viewer: Cesium.Viewer) {
 ...
 * */
latlng.formatPositon = function (position: Cesium.Cartesian3) {
...
//笛卡尔转经纬度/弧度
latlng.Cartesian3To = function (cartesian3: Cesium.Cartesian3, viewer: Cesium.Viewer) {
    if (cartesian3) {
        if (viewer) {
...
latlng.windingPoint = {
    isStart: false,
    viewer: null,
    start: function (viewer: Cesium.Viewer, point: any) {
        if (!point)
            point = latlng.getCenter(viewer);
    ...
    clock_onTickHandler: function (e: any) {
