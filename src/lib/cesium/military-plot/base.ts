...
import EvtListener from './EventListener';
...
export default class Base {
  ...
  eventHandler: CesiumTypeOnly.ScreenSpaceEventHandler | undefined;
  polygonEntity: CesiumTypeOnly.Entity | undefined | any;
  geometryPoints: CesiumTypeOnly.Cartesian3[] = [];
  state: State = 'drawing';
  controlPoints: CesiumTypeOnly.EntityCollection | any;
  // controlPoints: CesiumTypeOnly.EntityCollection | undefined;
  controlPointsEventHandler: CesiumTypeOnly.ScreenSpaceEventHandler | undefined;
  lineEntity: CesiumTypeOnly.Entity | undefined | any;
  type!: 'polygon' | 'line' | 'billboard';
  freehand!: boolean;
  style: GeometryStyle | undefined;
  outlineEntity: CesiumTypeOnly.Entity | undefined | any;
  eventDispatcher: EventDispatcher;
  dragEventHandler: CesiumTypeOnly.ScreenSpaceEventHandler | undefined;
  entityId: string = '';
  points: CesiumTypeOnly.Cartesian3[] = [];
  styleCache: GeometryStyle | any;
  minPointsForShape: number = 0;
  tempLineEntity: CesiumTypeOnly.Entity | undefined;
  _isDynamic: boolean = false;
  billboardEntity: CesiumTypeOnly.Entity | any;
  position: CesiumTypeOnly.Cartesian3 | any;

  constructor(cesium: any, viewer: CesiumTypeOnly.Viewer, style?: GeometryStyle) {
    this.cesium = cesium;
    this.viewer = viewer;
  ...
  setState(state: State) {
    this.state = state;
    this.isDynamic = !(state === 'static' || state === 'hidden');
  ...
  onClick() {
    ...
    this.eventHandler.setInputAction((evt: any) => {
      ...
      if (this.type === 'billboard') {
        activeEntity = this.billboardEntity;
      }
      ...
      if (this.state === 'drawing') {
        ...
        if (!hitEntities || activeEntity!.id !== pickedObject.id.id) {
          ...
          // EvtListener.dispatchEvent('editEnd', this.getPoints());
        ...
        if (hitEntities && activeEntity!.id === pickedObject.id.id) {
          let pickedGraphics: any;
          if (this.type === 'polygon') {
            pickedGraphics = pickedObject.id.polygon;
          } else if (this.type === 'line') {
            pickedGraphics = pickedObject.id.polyline;
          } else if (this.type === 'billboard') {
            pickedGraphics = pickedObject.id.billboard;
          }
          ...
          if (this.cesium.defined(pickedGraphics)) {
            ...
            // EvtListener.dispatchEvent('editStart')
          }
        }
      }
  ...
  finishDrawing() {
    ...
    this.addControlPoints();
    this.draggable();
    const entity = this.polygonEntity || this.lineEntity || this.billboardEntity;
    ...
    EvtListener.dispatchEvent('drawEnd', entity);
  }
  ...
  set isDynamic(value: boolean) {
    ...
    if (!value) {
      this.polygonEntity.polygon!.hierarchy = new this.cesium.PolygonHierarchy(this.geometryPoints) as any;
      this.outlineEntity!.polyline!.positions = [...this.geometryPoints, this.geometryPoints[0]] as any;
    } else {
      this.polygonEntity.polygon!.hierarchy = new this.cesium.CallbackProperty(() => {
        return new this.cesium.PolygonHierarchy(this.geometryPoints);
      }, false);
      this.outlineEntity!.polyline!.positions = new this.cesium.CallbackProperty(() => {
  ...
  addControlPoints() {
    ...
    let draggedIcon: CesiumTypeOnly.Entity | any = null;
    let dragStartPosition: CesiumTypeOnly.Cartesian3 | undefined;
    ...
    this.dragEventHandler.setInputAction((event: any) => {
      if (dragging && startPosition) {
        ...
        if (newPosition) {
          ...
          this.position = this.cesium.Cartesian3.add(this.position, translation, new this.cesium.Cartesian3());
          ...
          this.controlPoints.map((p: CesiumTypeOnly.Entity | any) => {
            const position: any = p.position?.getValue(this.cesium.JulianDate.now());
  ...
  showWithAnimation(duration: number = 2000, delay: number = 0, callback?: () => void) {
    ...
    if (this.type === 'polygon') {
      let alpha = 0.3;
      const material: any = this.styleCache!.material;
      if (material.image) {
        // With Texture
  ...
  animateOpacity(
    ...
    setTimeout(() => {
      const graphics: any = entity.polygon || entity.polyline || entity.billboard;
  ...
  private getNewPosition(curveControlPoints: any, t: any) {
    ...
    let curvePoints: any = Utils.getCurvePoints(0.3, curveControlPoints);
    curvePoints = curvePoints.map((p: any) => {
  ...
  private interpolateAlongCurve(curvePoints: any, t: any) {
