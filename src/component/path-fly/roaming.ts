...
export class Roaming {
    ...
    /**
       ...
       */
    updateOptionsParams(options: { view: any; model: {}; time: any; data: any; speed: number; isPathShow: any; }) {
    ...
    processData(data: any[]) {
        ...
        data.forEach((position: any) => {
            const car3Position = Cesium.Cartesian3.fromDegrees(position.lon, position.lat, position.height); // 给定默认值
    ...
    /**
       ...
       * @returns
       */
    computeRoamingLineProperty(coordinates: string | any[], time: number) {
    ...
    /**
       ...
       * @memberof Roaming
       */
    createEntity(position: any, start: any, stop: any, isPathShow: any) {
    ...
    /**
       ...
       * @memberof Roaming
       */
    setRoamingPathVisibility(visible: any) {
        if (this.entity) {
    ...
    /**
       ...
       * @memberof Roaming
       */
    setRoamingModelVisibility(visible: any) {
        if (this.entity) {
    ...
    /**
       ...
       * @memberof Roaming
       */
    setCameraPosition(position: any, options: { pitch: any; range: number; }) {
        if (position) {
            // 最新传进来的坐标（后一个位置）
    ...
    /**
       ...
       * @param callback
       */
    addSceneEvent(callback: { (time: any): void; (time: any): void; (arg0: any): void; }) {
        ...
        this.handler = this.viewer.scene.preRender.addEventListener((scene: any, time: any) => {
            callback(time);
        });
    }
