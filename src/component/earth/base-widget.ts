...
export default abstract class BaseWidget extends HTMLElement {
    ...
    async connectedCallback() {
        ...
        this.className +=
            " " + (this.manifest.className || this.manifest.tagName || "");
    ...
    #getFunction(values: string[]) {
        ...
        if (!this.#cachedFunctions.has(key)) {
            ...
            if (
                ...
                str.includes("setTimeout") ||
                str.includes("setInterval")
            ) {
    ...
    #getTagNode(astNode: ASTNode, loop: Loop): VNode {
        ...
        Object.keys(astNode.attrs || {}).forEach((name: string) => {
            ...
            // 处理特殊属性
                ...
                ["s-checked", "s-disabled", "s-readOnly", "s-selected"].includes(name)
            ) {
    ...
    #toVNode(
        ...
        const processNode = (node: ASTNode, loop: Loop) => {
            if (node.type === "tag") {
                ...
                const content: any = node.content?.replace(
                    interpolationRegex,
                    (...args: string[]) => {
        ...
        const processFor = (node: ASTNode, loop: Loop) => {
            ...
            const func = this.#getFunction([
                "$data",
                ...loop.keys,
                `return (${node.attrs.each})`,
            ]); // 构造遍历数组的函数
            let arr = func(this.$data, ...loop.values) || [];
            let i = 0;
            arr.forEach((value: any) => {
                const loopCopy = {
                    keys: [...loop.keys, key, index],
                    values: [...loop.values, value, i],
                };
