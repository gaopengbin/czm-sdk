...
// 组件的模板
import template from './widget-panel.html?raw';
 ...
 */
@Component({
    tagName: 'webgis-widget-panel',
    className: 'webgis-widget-panel',
    template: template,
    hasConfig: true
})
export default class WidgetPanel extends BaseWidget {
    #panel: any = null;
    ...
    initPanel() {
        ...
        this.$data.icon = this.config.icon || 'bi-stack';
        this.$data.title = this.config.label || '';
