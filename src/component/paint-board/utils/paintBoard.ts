...
export class PaintBoard {
  ...
  initCanvasStorage() {
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        const file = files?.find((item:any) => item?.id === currentId)
        if (file && this.canvas) {
          this.canvas.clear()
  ...
  copyObject() {
    ...
    const copys = targets.map((target:any) => {
      return new Promise<fa.FabricObject>((resolve) => {
        target?.clone((cloned: fa.FabricObject) => {
          const id = uuidv4()
          cloned.set({
